...
function bmi(weight, height) {
  ...
  else if (bodyMassIndex <= 25) return "Normal";
  else if (bodyMassIndex <= 30) return "Overweight";
  else return "Obese";
}
...
//Дан массив целых чисел, найдите то, которое встречается нечетное количество раз. Всегда будет только одно целое число, которое встречается нечетное количество раз.
function findOdd(A) {
  const res = [];

  const obj = A.reduce( (key, value) => {
    key[value] = (key[value] || 0) + 1 ;
    return key;
  } , {}); //{ '1': 2, '2': 4, '3': 6, '4': 1 } //получил количество каждой цифры

  Object.entries(obj).map(e => { //[ [ '1', 2 ], [ '2', 4 ], [ '3', 6 ], [ '4', 1 ] ] сделал массив для перебора значений
    if (e[1] % 2 !== 0) {
      res.push(e[0])
  }
})
